...
GaussianFunctionPainter::GaussianFunctionPainter(const int x0, const int y0, const float sigmaX, const float sigmaY):x0(x0),y0(y0),sigmaX(sigmaX),sigmaY(sigmaY){
...
void GaussianFunctionPainter::draw(QImage& referenceImage, const QColor presetColor){
    ...
    for(int y_offset = 0; y_offset < referenceImage.height(); y_offset++){
        for(int x_offset = 0; x_offset < referenceImage.width(); x_offset++){
            ...
            QColor color = QColor(255*(1-value)+presetColor.red()*value,255*(1-value)+presetColor.green()*value,255*(1-value)+presetColor.blue()*value);
